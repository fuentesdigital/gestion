...
abstract class backuprestore
{
	...
	*/
	...
	final public function __construct($host=SERVER_NAME,$user=DB_USER_NAME,$pass=DB_PWD)
	{
	...
	final public function setDbs($db=DB_NAME)// con * recorre todo el sistema de base de datos
	{
		$db=trim($db);
		if (empty($db) || $db == '*'){
	...
	*/
	...
	abstract protected function backupData($cmp='1',$ftp='0',$fhost='',$fpass='',$fport='21');
...
***********************';               // mohamed ghareeb mohamed saeed signature
	...
	*/
	...
	public function backupData($cmp='1',$ftp='0',$fhost='',$fuser='',$fpass='',$fport='')
	{
	...
	*/
	...
	// ignore database dump complete message
		...
		foreach ($tt=explode(self::FIELDSEP,$this->text) as $query){
			if (empty($query))
			continue;
			$rs=mysqli_query($query,$this->link);
			if(!$rs){
			...
			if (strstr($query,"CREATE DATABASE")){
				...
				mysqli_select_db($seldb,$this->link);
				//**
