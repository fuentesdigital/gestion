<?php
class graficos { // Graficos de Barras
    public $imgpath;
    public $bt_lgtitle;
    public $graf_height;
    public $bwidth;
    ...
    public $autoshrink;
    public $legendx_url;
    public $legendx_onClick;
    public $legendy_url;
    public $legendy_onClick;
    public $cell_url;
    public $legendx_id;
    public $data;
    public $legendx;
    ...
    public $drawempty_x;
    public $drawempty_y;
    public $asbarchart_csshown;

    public $grafica_out;

    public function __construct() {
        $this->grafica_out = '';
        $this->imgpath = 'clases/graficos_img/'; // place all 'diagram' images in this "folder";
        $this->bt_lgtitle = '';
        $this->graf_height = 240;
        $this->bwidth = 0;
        $this->precision  = 2;
        $this->bt_total   = 'Totals';
        $this->showtotals = 1;
        $this->btilemode = 0;
        $this->showdigits = 1; // in the bottom show digits by default
        $this->autoshrink = 1024; // auto-adjust graph width to not greater than this value (pixels)
        $this->legendx_url = ''; // URL with {ID} macro. If not empty, legend texts on X-axis become "hrefs"
        $this->legendx_onClick = ''; // onClick event string, for the URL above, (with {ID} macro)
        $this->legendy_url = ''; // URL with {ID} macro. If not empty, legend texts on Y-axis become "hrefs"
        $this->legendy_onClick = ''; // onClick event string, for the URL above, (with {ID} macro)
        $this->cell_url = ''; // tamplate for cell-URLs : {X}, {Y} will be subst-ed with current x,y "legend" values
        $this->legendx_id = 0; // here must be an array of ID values for all x URL's. If not set, titles will be used
        $this->data = array();
        $this->legendx = array(); // for auto-making legend from SQL query
        $this->legendy = array();
        $this->ShowPercents = array(); // one element: showPercents['legend_y'] = "title" - as percent after [n2] row
        $this->debug = 0; // show debug info
        $this->drawempty_x = 1; // if 0, don't draw zero columns
        $this->drawempty_y = 1; // if 0, don't draw zero bars and "rows" in digit part
        $this->asbarchart_csshown = 0;
    }
    ...
    function HideEmptyX($val=true) {
        $this->drawempty_x = !($val);
    }
    ...
    function InitData($legend_x=0, $legend_y=0) { // clears all gathered data. If legends passed, fills (X x Y) with 0 values
        $this->data = array();
    ...
    function DiagramBar($legend_x='', $legend_y='', $dtarray=0, $data_title='',$domid='') {
        ...
        if(empty($asbarchart_csshown)) { //<2>
            $asbarchart_csshown = 1;
            $this->grafica_out .=  '<STYLE TYPE="text/css"><!--';
            if(!empty($this->btilemode)) { //  tiled picture arrenged with css
                for($ii=0; $ii<count($bar); $ii++) {
                    $img = $this->imgpath.$bar[$ii];
                    $this->grafica_out .=  "  td.tbar{$ii} {  background-image: url($img); background-repeat: repeat; }\n";
                    }
                }
                $this->grafica_out .= "-->\n</STYLE>";
            ...
            $this->grafica_out .= "<DIV id='$wholeid' $recuadro_dinamico >";
            if(!empty($data_title))
                $this->grafica_out .= "<div align='center'><font size='2.4px' class='titulo_seccion'>$data_title</font></div>\n<p>";
            ...
            if(empty($this->drawempty_y)) { // will draw only non-zero Y-columns
                ...
                for($ky=0;$ky<count($legend_y); $ky++) {
                    for($kx=0;$kx<count($legend_x);$kx++) {
                        if(isset($data[$kx][$ky]) && floatval($data[$kx][$ky])!=0) {
                            $draw_cy[$ky]=1;
                            $cnt_y++;
                            break;
                        }
            ...
            if( empty($this->bwidth)) { // compute optimal width for one bar
            ...
            $this->grafica_out .= "<!-- bar outline table-->\n<P align=center>";
            $this->grafica_out .= "<table name='001' width='$gr_width' border=0 cellspacing=1 cellpadding=0>\n";

            // first row (+) - is a main, Y axis, charts and legend if needed
            if($steps_pos>0) { //<2>- pos_parts>
                $pos_h = floor($height_q*$steps_pos);

                $this->grafica_out .= "<tr height=$pos_h><td valign=top>\n";

                // sub-table for Y axis
                $this->grafica_out .= "<table width='100%' sname=002 border=0 cellspacing=0 cellpadding=0>\n";
                for ($kk=1; $kk<=$steps_pos; $kk++) {
                    ...
                    $this->grafica_out .= "<tr class='$cls' valign=top height='$height_q'><td width='100%' nowrap valign=top align=right><b>$fNo</b></td></tr>\n";
                }
                $this->grafica_out .= "</table></td><!-- Y(+) axis done -->\n";

                $this->grafica_out .= "<!-- (+)main graphics area maxval = $maxval-->\n";

//    if($maxval<1) $maxval=2; // TODO: values less than 1 - 0.005... ???!!!
                for ($ix=0; $ix < count($legend_x); $ix++) {
                    if(empty($draw_cx[$ix])) continue;
                    $this->grafica_out .= "<Td nowrap align='middle' valign='bottom' style=\"background-image: url($bgpic); background-repeat: repeat;\">
      <table border=0 cellspacing=0 cellpadding=0><tr valign='bottom'>\n";
                    for($iy=0; $iy < count($legend_y); $iy++) { //<4>
                        if(empty($draw_cy[$iy])) continue;
                        $pc = $bar[$iy % count($bar)];
                        $value = empty($data[$ix][$iy]) ? 0 : floatval($data[$ix][$iy]);
                        $hght = floor($value * $height_q * $steps_pos / $maxval);
//        $this->grafica_out .= "height[$ix,$iy] = $hght = ($value * $height_q * $steps_pos / $maxval )<br>"; // debug
            ...
            width='{$this->bwidth}' height='$hght' border=1 bordercolor=black></td></tr></table>");
                        if(!empty($this->cell_url)) {
                            $l_x = $this->AsGetRowKey($legend_x[$ix]);
                            $l_y = $this->AsGetRowKey($legend_y[$iy]);
                            $ato = array( $l_x, $l_y);
                            $onebar = "<a href='".(str_replace(array('{X}','{Y}'),$ato, $this->cell_url))."'>$onebar</a>";
                        }
                        if($hght>0) {
                            $this->grafica_out .= empty($this->btilemode)? "<td>$onebar</td>" : "<table cellspacing=0 border=0 cellpadding=0><tr><td class='tbar$iy'>$onebar</td></tr></table>";
                        }
                        else { // draw "empty" block with the same width
                            $this->grafica_out .= "<td><img width='$ewidth' height='1' border='0'></td>";
                        }
                    } //<4>
                    $this->grafica_out .= " </tr></table>"; // inner table for bars
                    $this->grafica_out .= "</Td>";
                }
                $this->grafica_out .= "\n<!-- (+)main graphics area finished-->\n";

                $legend_drawn = true;
                $this->grafica_out .= "<!-- right side-legend area --><td nowrap valign=top width=0 align=center class='barhead'>\n";
                if(empty($this->showdigits)) { //<3>
                    $this->grafica_out .= "  <table name='legend' width=0 border=0>";
                    for ($iy=0; $iy<count($legend_y); $iy++) {
                        if(empty($draw_cy[$iy])) continue;
                        $pc = $bar[$iy % count($bar)];
                        $lgd = AsGetRowValue($legend_y[$iy]);
                        $this->grafica_out .= "   <tr><td nowrap><img src='{$this->imgpath}{$pc}' width='{$this->bwidth}' height=12 border=1 bordercolor=black></td><td nowrap>$lgd</td></tr>\n";
                    }
                    $this->grafica_out .= "  </table><!-- legend -->\n";
                } //<3>
                $this->grafica_out .= "</td><!-- main Legend area finished-->\n";
            ...
            if($steps_neg>0) { // <2>-draw negative values area
                $neg_h = floor($height_q*$steps_neg);
                $absmin = abs($minval);
                $this->grafica_out .= "<tr height=$neg_h class='barhead'><td valign=top>\n";
                ...
                for ($kk=1; $kk<=$steps_neg; $kk++) {
                    ...
                    $fNo = ($nNo == floor($nNo)) ? number_format($nNo) : number_format($nNo,$this->precision);
                    $this->grafica_out .= "<tr class='$cls' valign=top height='$height_q'><td nowrap width='100%' valign=bottom align=right><b>-$fNo</b></td></tr>\n";
                }
                $this->grafica_out .= "</table></td><!-- Y(-) axis done -->\n";
                // Y(-) axis drawn - Y-axis done, now we'll draw area with the bar charts

                $this->grafica_out .= "<!-- (-)main graphics area minval = $minval-->\n";

//    if($absmin<1) $absmin=2; // TODO: values less than 1 - 0.005... ???!!!
                for ($ix=0; $ix < count($legend_x); $ix++) {
                    if(empty($draw_cx[$ix])) continue;
                    $this->grafica_out .= "<td nowrap __width='$tdwidth' align='middle' valign=top
       style=\"background-image: url($bgpic); background-repeat: repeat; \">\n";

                    $this->grafica_out .= " <table border=0 cellspacing=0 cellpadding=0><tr valign=top>\n";
                    for($iy=0; $iy < count($legend_y); $iy++) {
                        if(empty($draw_cy[$iy])) continue;
                        $pc = $bar[$iy % count($bar)]; // todo - $iy % count($bar);
                        $value = empty($data[$ix][$iy]) ? 0 : floatval($data[$ix][$iy]);
                        $hght = -1 * floor($value * $height_q * $steps_neg / $absmin);
//        $this->grafica_out .= "height $hght = ($value * $steps_pos * $Ystep / $maxval )<br>"; // debug
                        if($hght>0) {
                            $this->grafica_out .= empty($this->btilemode) ? "<td><img src='{$this->imgpath}$pc' width=$this->bwidth height=$hght border=1 bordercolor=black></td>"
                    ...
                    $this->grafica_out .= " </tr></table>\n</td>";
                }

                $this->grafica_out .= "\n<!-- (-)main graphics area finished-->\n";

                $this->grafica_out .= "<td valign=top width=0 align=center  class='barhead'><!-- legend area -->\n";
                if(empty($legend_drawn) && empty($this->showdigits)) { // <3-legend in neg.area>
                    $this->grafica_out .= "  <table name='legend' width=0 border=0>";
                    for ($iy=0; $iy<count($legend_y); $iy++) {
                        if(empty($draw_cy[$iy])) continue;
                        $pc = $bar[$iy % count($bar)];
                        $lgd = AsGetRowValue($legend_y[$iy]);
                        $this->grafica_out .= "   <tr><td><img src='{$this->imgpath}$pc' width=$this->bwidth height=12 border=1 bordercolor=black></td><td>$lgd</td></tr>\n";
                    }
                    $this->grafica_out .= "  </table><!-- legend -->\n";
                } // <3-legend in neg.area>
//    else $this->grafica_out .= "<table width=0 border=0><tr><td></td></tr></table>";
...
// values in numeric form, and totals column
            $this->grafica_out .= "<tr height=4><td colspan=$cspan_x class='barhead'><img height=4></td></tr>"; // for nicer look

            $this->grafica_out .= "<tr class='barhead'><td class='head'>$this->bt_lgtitle</td>"; // left bottom td - legend y titles
            for($ix=0; $ix<count($legend_x); $ix++) { //<2>
                // $legendx_url, $legendx_onClick - use them !
                if(empty($draw_cx[$ix])) continue;
                $id_x  =  $this->AsGetRowKey($legend_x[$ix]); # is_array($legend_x[$ix])? $legend_x[$ix][0] : $legend_x[$ix];
                $ltext =  $this->AsGetRowValue($legend_x[$ix]);
...
#    else $ltext = $id_x;
                ...
                $this->grafica_out .= "<td class='barhead' nowrap>$ltext</td>\n";
            } //<2>
            $this->grafica_out .= "<td class='barhead'>".( (($this->showtotals & 1) && $this->showdigits) ? $this->bt_total :'')."</td></tr>\n"; // ��� ������ ��������
//$this->grafica_out .= "<table border=0><tr><td align=right>����-�</td></tr>\n";
            if($this->showdigits) { //<2> output number presentation of samples
                $cls = 'barodd';
                for ($iy=0; $iy<count($legend_y); $iy++) { //<3>
//   $pc = $bar[$iy]; // todo - $iy % count($bar);
                    if(empty($draw_cy[$iy])) continue;
                    $summs = 0;
                    $y_id =  $this->AsGetRowKey($legend_y[$iy]);
                    $lgd =  $this->AsGetRowValue($legend_y[$iy]);
                    ...
                    $this->grafica_out .= "   <tr class='$cls'><td nowrap><img src='$img' width=$this->bwidth height=12 border=1 bordercolor=black> $lgd</td>\n";
                    for($ix=0; $ix<count($legend_x); $ix++) {
                        if(empty($draw_cx[$ix])) continue;
                        $value = empty($data[$ix][$iy]) ? 0 : number_format($data[$ix][$iy], $this->precision);
                        $summs += empty($data[$ix][$iy])? 0 : $data[$ix][$iy];
                        $this->grafica_out .= "   <td align=right nowrap>&nbsp; $value &nbsp;</td>\n";
                    }
                    $smm = ($this->showtotals & 1) ? number_format($summs, $this->precision): '';
                    $this->grafica_out .= "<td align=right nowrap><b>&nbsp; $smm &nbsp;</b></tr>\n";

                    if(!empty($this->ShowPercents[$lgd]) && ($iy>=1) ) { //<4> draw data[n-1]/data[n]*100 in percents
                        $y2 = $iy; // last row
                        ...
                        $cls = ($cls=='bareven') ? 'barodd' : 'bareven';
                        $this->grafica_out .= "   <tr class='$cls'><td nowrap>$prcttl</td>\n"; // title "percents"
                        $sum1 = 0;
                        ...
                        $this->grafica_out .= "<td align=right nowrap><b> $prctotal &nbsp; </b></tr>\n";
                    } //<4>

                } //<3>

                if(($this->showtotals & 2)) { // <4> count 'horizontal' totals
                    $cls = ($iy % 2) ? 'barodd' : 'bareven';
                    $this->grafica_out .= "   <tr class='$cls'><td nowrap>$this->bt_total</td>\n"; // title "totals"
                    for($ix=0; $ix<count($legend_x); $ix++) {
                        if(empty($draw_cx[$ix])) continue;
                        $value = 0;
                    ...
                    $this->grafica_out .= "<td nowrap><b>&nbsp;</b></tr>\n";
                } //<4>
            } //<2> - $this->showdigits

            $this->grafica_out .= "</table><!-- 001 finish -->\n</p></DIV>";
            // bar chart drawn


        } // DiagramBar() end

        function GatherData($sqlquery, $legend_x='', $legend_y='', $position_y=-1) { // runs sql query and places all needed data into array for drawing
            // if legend_* array not passed, SQL query will be used to make them
//    $this->grafica_out .= "GatherData: $sqlquery<br>"; // debug
            $onecol = array();
            ...
            if($this->debug) {
                if($res === false)
                    $this->grafica_out .= "GatherData query error, qry: $sqlquery<br>Error:".mysql_error();
                else
                    $this->grafica_out .= "GatherData: query : $sqlquery<br>returned rows:".mysql_affected_rows().'<br>';
            }
            $cur_x = '-?-';
            $cur_y = '-?-';
            $x_pos = 0;
            $y_pos = 0;
            if($res) { //<3>
                while(($rw = mysql_fetch_row($res))) { //<4>
                    $rcnt = count($rw);
                    ...
                    if(is_array($legend_y) && count($legend_y)>0) { //<5>
                        $y_pos = -1;
                        if($rcnt<3 || $position_y>=0) {
                            $y_pos = $position_y;
                        }
                        else { //<6>
//             $summa = $rw[2];
                            for($kk=0; $kk<$leny; $kk++) { //<7>
//                 if($this->debug) $this->grafica_out .= "test y: $kk ==".$legend_y[$kk].'/'.$rw[1]."<br>\n";
                        ...
                        if($cur_y !== $rw[1]) { //<6>
                            ...
                            if($y_pos>=count($this->legendy)) {// <7>add new title to 'internal' legend-y
                                $this->legendy[] = $cur_y;
...
/*           else {
                if($this->debug) $this->grafica_out .= "wrong position for Data in ".$rw[0].($rcnt>2 ? ','.$rw[1] : '').'<br>';
           }
*/
//           else echo "nowhere to put data for [".$rw[0]."][".$rw[1]."]<br>"; // debug
                    if($this->debug) $this->grafica_out .= "[$cur_x,$cur_y] [$x_pos][$y_pos] = $summa<br>\n";
                } //<4>
            } //<3>
            else $this->grafica_out .= "GatherData: Error in query : $sqlquery<br>error : ".mysql_error();
//    var_dump($this->legendx);  var_dump($this->legendy); // debug
            return $this->data;
        } //<GatherData() function end        // end class definition graficos
        public function __destruct() {
            unset($this->HideEmptyXY);
            unset($this->HideEmptyY);
            unset($this->HideEmptyX);
            unset($this->InitData);
            unset($this->SetImagePath);
            unset($this->DiagramBar);
            unset($this->GatherData);
            unset($this->AsGetRowValue);
            unset($this->AsGetRowKey);
            unset($this->grafica_out);
            unset($this->imgpath);
            unset($this->bt_lgtitle);
            unset($this->graf_height);
            unset($this->graf_height);
            unset($this->bwidth);
            unset($this->precision);
            unset($this->bt_total);
            unset($this->showtotals);
            unset($this->btilemode);
            unset($this->showdigits);
            unset($this->autoshrink);
            unset($this->legendx_url);
            unset($this->autoshrink);
            unset($this->legendx_url);
            unset($this->legendx_onClick);
            unset($this->legendy_url);
            unset($this->legendy_onClick);
            unset($this->cell_url);
            unset($this->legendx_id);
            unset($this->data);
            unset($this->legendx);
            unset($this->legendy);
            unset($this->debug);
            unset($this->ShowPercents);
            unset($this->drawempty_x);
            unset($this->drawempty_y);
        }
    }
    ?>
